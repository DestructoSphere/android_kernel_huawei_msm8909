 ...
 */
//use synaptics_dsx_esd_start/stop when fwup, and fix some logs
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
    ...
    if (fwu->force_update) {
        tp_log_warning("%s#%d: force_update=%d\n",
    ...
    if (fwu->in_flash_prog_mode) {
        tp_log_warning("%s#%d: in_flash_prog_mode=%d\n",
    ...
    tp_log_warning("%s#%d: Device firmware ID = %d\n",
    ...
    if (header->contains_firmware_id) {
        ...
        if (!strptr) {
            tp_log_err("%s#%d:No valid PR number (PRxxxxxxx) "
        ...
        kfree(firmware_id);
        if (retval) {
            tp_log_err("%s#%d: obtain img firmware ID fail!\n",__func__,__LINE__);
            flash_area = NONE;
            goto exit;
        }
    }
    tp_log_warning("%s#%d: img firmware ID = %d\n",
    ...
    if (image_fw_id > device_fw_id) {
        ...
        tp_log_warning("%s#%d: img firmware ID older than device firmware ID\n",
                __func__,__LINE__);
        flash_area = NONE;
        goto exit;
    }
    ...
    if (retval < 0) {
        tp_log_err("%s#%d: read devcie config ID fail!\n",__func__,__LINE__);
        flash_area = NONE;
        goto exit;
    }
    device_config_id = extract_uint_be(config_id);
    tp_log_warning("%s#%d: Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
            __func__,__LINE__,config_id[0],config_id[1],config_id[2],config_id[3]);

    /* Get image config ID */
    image_config_id = extract_uint_be(fwu->config_data);
    tp_log_warning("%s#%d: Image config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
            __func__,__LINE__,
...
exit:
    if (flash_area == NONE) {
        tp_log_warning("%s#%d: No need to do reflash!\n",__func__,__LINE__);
    } else {
        tp_log_warning("%s#%d:Updating %s!\n",__func__,__LINE__,
    ...
    for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
        ...
        else
        {
            tp_log_info( "%s: Read addr(0x%02x) fn_number is zero!\n",
                __func__,addr);
            if ((F01_QUERY_BASE == addr) || (F34_QUERY_BASE == addr))
            {
#ifdef CONFIG_HUAWEI_DSM
...
static int fwu_do_reflash(void)
{
    ...
    tp_log_warning("%s#%d: UI update start...\n",__func__,__LINE__);

    retval = fwu_enter_flash_prog();
    if (retval < 0) {
        return retval;
    }
    tp_log_warning("%s#%d:Enter in flash prog mode\n",__func__,__LINE__);

    retval = fwu_write_bootloader_id();
    if (retval < 0) {
        tp_log_err("%s#%d:Bloader ID write err, ret=%d\n",__func__,__LINE__, retval);
        return retval;
    }
    tp_log_warning("%s#%d:Bootloader ID written\n",__func__,__LINE__);

    retval = fwu_write_f34_command(CMD_ERASE_ALL);
    if (retval < 0) {
        tp_log_err("%s#%d:Erase cmd err, ret=%d\n",__func__,__LINE__, retval);
        return retval;
    }
    tp_log_warning("%s#%d:Erase all command written\n",__func__,__LINE__);

    retval = fwu_wait_for_idle(ERASE_WAIT_MS);
    if (retval < 0) {
        tp_log_err("%s#%d:wait idle status err, ret=%d\n",__func__,__LINE__, retval);
        return retval;
    }
    tp_log_warning("%s#%d:Idle status detected\n",__func__,__LINE__);

    if (fwu->firmware_data) {
        retval = fwu_write_firmware();
        if (retval < 0) {
            tp_log_err("%s#%d:Fw prog err, ret=%d\n",__func__,__LINE__, retval);            
            return retval;
        }
        tp_log_warning("%s(line %d): Firmware programmed\n", __func__,__LINE__);
    } else {
        tp_log_warning("%s#%d: firmware_data is NULL!\n",__func__,__LINE__);
    }
    ...
    if (fwu->config_data) {
        ...
        tp_log_warning("%s#%d: config_data is NULL!\n",__func__,__LINE__);    
    }
...
static int fwu_do_write_config(void)
{
    ...
    tp_log_warning("%s#%d: CONFIG update start...\n",__func__,__LINE__);
...
static int fwu_start_write_config(void)
{
    ...
    tp_log_info("%s#%d: write config process begin\n", __func__,__LINE__);

    retval = fwu_do_write_config();
    if (retval < 0) {
        tp_log_err("%s#%d: write config fail!\n",__func__,__LINE__);
    }

    fwu->rmi4_data->reset_device(fwu->rmi4_data);

    tp_log_info("%s#%d: write config process done\n", __func__,__LINE__);
...
/*To optimize the method of getting the module name*/
    ...
    else if(0 == strcasecmp(product_id, FW_TIANMA_STR))
    {
        return "tianma";
    }
    else
...
static void synaptics_set_appinfo(void)
{
    ...
    snprintf(touch_info,sizeof(touch_info),"synaptics_dsx_%s.%d",get_cof_module_name(config_id[2]),config_id[3]);
#ifdef CONFIG_APP_INFO
...
static int fwu_start_reflash(void)
{
    ...
    if (fwu->rmi4_data->sensor_sleep) {
        tp_log_err("%s#%d:Sensor sleeping\n", __func__,__LINE__);
        return -ENODEV;
    }

    fwu->rmi4_data->staying_awake = true;

    tp_log_info("%s#%d: start...\n", __func__,__LINE__);
    ...
    if (fwu->ext_data_source) {
...
#endif /*CONFIG_HUAWEI_KERNEL*/
        tp_log_warning("%s#%d:Requesting firmware image %s\n",
                __func__,__LINE__, fwu->image_name);
        ...
        for (retry = 0; retry < SYN_FW_RETRY_TIMES; retry++) {
            ...
            if (retval != 0) {
                tp_log_err("%s#%d: Firmware image %s not available\n",
                        __func__,__LINE__, fwu->image_name);
                retval = -EINVAL;
                goto exit;
            }
            ...
            if (fw_entry->data)
            {
                ...
                tp_log_warning("%s#%d: fw_entry->data is NULL\n",__func__,__LINE__);
            }
            tp_log_info("%s#%d: request_firmware retry %d\n",
        ...
        tp_log_info("%s#%d: Firmware image %s is get successful!\n",
                __func__,__LINE__, fwu->image_name);
    #ifdef CONFIG_64BIT
        tp_log_info("%s(line %d): "
    ...
    tp_log_info("%s#%d: fwu->bl_version = %d"
    ...
    if (fwu->bl_version != header.bootloader_version) {
        tp_log_err("%s#%d:Bootloader version mismatch\n",__func__,__LINE__);
        retval = -EINVAL;
        goto exit;
    }
    ...
    retval = fwu_read_f01_device_status(&f01_device_status);
    if (retval < 0) {
        tp_log_err("%s#%d:ret=%d, read_f01_device_status fail!\n",
            __func__,__LINE__,retval);
        goto exit;
    }

    if (f01_device_status.flash_prog) {
        tp_log_warning("%s#%d: In flash prog mode\n",__func__,__LINE__);
    ...
    if (fwu->do_lockdown) {
        ...
        case V6:
            ...
            if (retval < 0) {
                tp_log_err("%s#%d:lockdown failed!\n", __func__,__LINE__);
            }
        default:
            break;
        }
    ...
    if (header.firmware_size) {
        ...
        tp_log_err("%s#%d: firmware_size = 0\n",__func__,__LINE__);
    }
    if (header.config_size) {
        fwu->config_data = fw_image + FW_IMAGE_OFFSET +
                header.firmware_size;
    } else {
        tp_log_err("%s#%d: config_size = 0\n",__func__,__LINE__);
    }
    ...
    tp_log_info("%s#%d: ret=%d, reflash done\n",__func__,__LINE__,retval);

    fwu->rmi4_data->reset_device(fwu->rmi4_data);
    retval = fwu_scan_pdt();
    tp_log_info("%s#%d:ret=%d, fwu_scan_pdt done\n",__func__,__LINE__,retval);
...
exit:
    ...
    tp_log_warning("%s#%d:eflash process done, retval = %d\n",__func__,__LINE__,retval);
...
int synaptics_fw_upgrade(unsigned char *fw_data)
{
    ...
    if (!fwu || !fwu->initialized) {
        return -ENODEV;
    }
    
    synaptics_pm_rt_get(fwu->rmi4_data);
    ...
    synaptics_pm_rt_put(fwu->rmi4_data);
        ...
        char *buf, loff_t pos, size_t count)
{
    ...
    #ifdef CONFIG_64BIT
        tp_log_info("%s: Not enough space (%lu bytes) in buffer\n",
                __func__, count);
    #else
        tp_log_info("%s: Not enough space (%d bytes) in buffer\n",
                __func__, count);
    #endif
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    if (!fwu->firmware_name) {
        dev_err(&rmi4_data->i2c_client->dev,
                "%s: Failed to alloc mem for firmware_name_used\n",
                __func__);
        retval = -ENOMEM;
        goto exit_free_fn_ptr;
    }
    ...
    tp_log_info("%s: F01 product info: 0x%04x 0x%04x\n",
