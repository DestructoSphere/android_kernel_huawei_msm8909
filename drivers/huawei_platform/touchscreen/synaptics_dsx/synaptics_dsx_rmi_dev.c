...
#define DEVICE_CLASS_NAME "rmidev_dsx"
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
    ...
    if (IS_ERR(dev_data)) {
        tp_log_err("%s: Pointer of char device data is invalid", __func__);
        return -EBADF;
    }
...
static int rmidev_create_device_class(void)
{
    ...
    if (IS_ERR(rmidev_device_class)) {
        tp_log_err("%s: Failed to create /dev/%s\n",
...
static int __init rmidsx_dev_module_init(void)
{
    synaptics_rmi4_new_func(&rmidev_module, true);

    return 0;
}

static void __exit rmidsx_dev_module_exit(void)
{
    synaptics_rmi4_new_func(&rmidev_module, false);
...
module_init(rmidsx_dev_module_init);
module_exit(rmidsx_dev_module_exit);
