		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (fw_revctrl_new != fw_revctrl_img) {
		tp_log_info( "%s: Image is different, will upgrade\n", __func__);
		return 1;
	}
...
static void _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
#ifdef CONFIG_HUAWEI_DSM
		g_tp_dsm_info.constraints_UPDATE_status = FWU_FW_CONT_ERROR_NO_FW;
#endif /*CONFIG_HUAWEI_DSM*/
	...
	if (!fw->data || !fw->size) {
#ifdef CONFIG_HUAWEI_DSM
		g_tp_dsm_info.constraints_UPDATE_status = FWU_FW_CONT_ERROR_DATA_ERROR;
#endif /*CONFIG_HUAWEI_DSM*/
...
#ifdef CONFIG_HUAWEI_DSM
		g_tp_dsm_info.constraints_UPDATE_status = FWU_FW_CONT_ERROR_HEAD_ERROR;
#endif /*CONFIG_HUAWEI_DSM*/
		...
		void *context)
{
	...
	if (!fw) {
#ifdef CONFIG_HUAWEI_DSM
		g_tp_dsm_info.constraints_UPDATE_status = FWU_FW_CONT_BUILTIN_ERROR_NO_FW;
#endif /*CONFIG_HUAWEI_DSM*/
	...
	if (!fw->data || !fw->size) {
#ifdef CONFIG_HUAWEI_DSM
		g_tp_dsm_info.constraints_UPDATE_status = FWU_FW_CONT_BUILTIN_ERROR_DATA_ERROR;
#endif /*CONFIG_HUAWEI_DSM*/
