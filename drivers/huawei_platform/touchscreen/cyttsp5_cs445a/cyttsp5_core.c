		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (cd->response_buf[HID_OUTPUT_RESPONSE_REPORT_OFFSET]
		...
		tp_log_err( "%s: HID output response, wrong report_id\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[4] != HID_OUTPUT_BL_SOP) {
		tp_log_err( "%s: HID output response, wrong SOP\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[size - 1] != HID_OUTPUT_BL_EOP) {
		tp_log_err( "%s: HID output response, wrong EOP\n",
			__func__);
		return -EPROTO;
	}
		...
		struct cyttsp5_hid_output *hid_output, bool raw)
{
	...
	if (param_size > 4) {
		tp_log_err( "%s: Invalid parameter size\n", __func__);
		return;
	}
...
static int cyttsp5_startup_(struct cyttsp5_core_data *cd, bool reset)
{
	...
	if(cd->transient_state) {
		tp_log_err("%s %d: repower ic because of esd error\n", __func__, __LINE__);
...
#endif
static int cyttsp5_read_mfg_val0_(struct cyttsp5_core_data *cd)
{
	u16 crc;
	int rc;
	u8 mfg_val0[5];
	...
	rc = cyttsp5_hid_output_read_conf_block_(cd, 0, 5, 1, mfg_val0, &crc);
	if (rc) {
