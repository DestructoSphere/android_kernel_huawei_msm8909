...
int apr_set_q6_state(enum apr_subsys_state state)
{
	...
	ad_logn("%s: setting adsp state %d\n", __func__, state);
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
			ad_logn("APR: Image is loaded, stated\n");
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
		ad_logn("APR: q6 image already loaded\n");
#else
		ad_logd("APR: q6 image already loaded\n");
#endif
	} else {
#ifdef CONFIG_HUAWEI_KERNEL
		ad_loge("APR: cannot load state %d\n", apr_get_q6_state());
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
			ad_logn("%s: Wait for modem to bootup\n", __func__);
#else
			     ...
			     void *_cmd)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
		ad_logn("M-Notify: Shutdown started\n");
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
		ad_logn("M-Notify: Shutdown Completed\n");
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
		ad_logn("M-notify: Bootup started\n");
#else
	...
	case SUBSYS_AFTER_POWERUP:
		...
		is_modem_up = 1;
#ifdef CONFIG_HUAWEI_KERNEL
		ad_logn("M-Notify: Bootup Completed\n");
#else
