...
/*  * Function: need_report
  * Description: message will be reported or ignored
  * Input: mask -- message mask
  *        fmt -- string
  * Return: true -- report, false -- do not report
**/
static bool need_report(int mask, va_list args, const char *fmt)
{
	bool ret = true;

	switch (mask) {
		case KCOLLECT_FREEZER_MASK: {
			static pid_t old_pid = -1;
			pid_t new_pid = -1;

			new_pid = va_arg(args, pid_t);
			if (new_pid == old_pid) {
				ret = false;
			} else {
				old_pid = new_pid;
