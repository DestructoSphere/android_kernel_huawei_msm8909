	...
	uint32_t *zr)
{
	...
	if (mbhc->skip_imped_detection) {
		ad_logd("%s: Skip imped detect RL %d ohm, RR %d ohm\n",
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	if (!wrk_complete && plug_type == MBHC_PLUG_TYPE_HEADSET) {
		ad_logn("%s: Headset already reported\n", __func__);
		goto enable_supply;
	}
	/* since we remove swap & hph type detection,no need to use wcd_is_special_headset */
#ifndef CONFIG_HUAWEI_KERNEL
...
static irqreturn_t wcd_mbhc_hs_ins_irq(int irq, void *data)
{
	...
	if (detection_type) {
		...
		if (result2 == 0x0A) {
			...
			if (result2 & 0x02) {
				mic_trigerred++;
				ad_logd("%s: Insertion MIC trigerred %d\n",
			...
			if (result2 & 0x08) {
				hphl_trigerred++;
				ad_logd("%s: Insertion HPHL trigerred %d\n",
...
determine_plug:
	 ...
	 * Setup for insertion detection.
	 */
	ad_logd("%s: Disable insertion interrupt\n", __func__);
...
static irqreturn_t wcd_mbhc_hs_rem_irq(int irq, void *data)
{
	...
	do {
		...
		result2 = (snd_soc_read(codec,
			MSM8X16_WCD_A_ANALOG_MBHC_ZDET_ELECT_RESULT));
		ad_logd("%s: check result2 for fake removal: %x\n",
			 __func__, result2);
		if ((!(result2 & 0x01)) &&
	...
	ad_logd("%s: headset %s actually removed\n", __func__,
		removed ? "" : "not ");
	...
	if (removed) {
		if (!result2) {
			...
			if (!(result2 & 0x02)) {
				mic_trigerred++;
				ad_logd("%s: Removal MIC trigerred %d\n",
					 __func__, mic_trigerred);
			}
			if (!(result2 & 0x08)) {
				hphl_trigerred++;
				ad_logd("%s: Removal HPHL trigerred %d\n",
					 __func__, hphl_trigerred);
			}
			if (mic_trigerred && hphl_trigerred) {
				/*
...
report_unplug:
	...
	ad_logd("%s: Report extension cable\n", __func__);
	wcd_mbhc_report_plug(mbhc, 1, SND_JACK_LINEOUT);
	/*
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			ad_logd("%s: Reporting long button release event\n",
				 __func__);
			wcd_mbhc_jack_report(mbhc, &mbhc->button_jack,
			...
			if (mbhc->in_swch_irq_handler) {
				...
				ad_logd("%s: Reporting btn press\n",
					 __func__);
				wcd_mbhc_jack_report(mbhc,
						     &mbhc->button_jack,
						     mbhc->buttons_pressed,
						     mbhc->buttons_pressed);
				ad_logd("%s: Reporting btn release\n",
					 __func__);
				wcd_mbhc_jack_report(mbhc,
						&mbhc->button_jack,
						0, mbhc->buttons_pressed);
