...
#define CODECDUMP_PATH "/data/log/CodecDump"
			...
			struct wcd9xxx_micbias_setting *micbias)
{
	...
	ret = of_property_read_u32(dev->of_node, prop_name,
			&micbias->cfilt1_mv);
	if (ret) {
		ad_dev_logi(dev, "Looking up %s property in node %s failed",
			prop_name, dev->of_node->full_name);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (dec_num == NULL) {
		ad_dev_loge(codec->dev, "%s: Invalid DEC selected\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	if (dec_num == NULL) {
		ad_dev_loge(codec->dev, "%s: Invalid DMIC\n", __func__);
		return -EINVAL;
	}
...
static void msm8x16_skip_imped_detect(struct snd_soc_codec *codec)
{
	...
	if (codec != NULL) {
		...
		ad_logd("%s: Codec pointer is NULL\n", __func__);
	}
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	dec_num = strpbrk(dec_name, "12");
	if (dec_num == NULL) {
		ad_dev_loge(codec->dev, "%s: Invalid Decimator\n", __func__);
		ret = -EINVAL;
		goto out;
	}
...
static int msm8x16_wcd_device_down(struct snd_soc_codec *codec)
{
	...
	ad_dev_logn(codec->dev, "%s: device down!\n", __func__);
#else
...
#ifdef CONFIG_HUAWEI_KERNEL
	ad_dev_logn(codec->dev, "%s: device up!\n", __func__);
#else
...
static int msm8x16_wcd_codec_probe(struct snd_soc_codec *codec)
{
    ...
    if(sys_access(CODECDUMP_PATH,0)!=0)
    {         
        if(sys_mkdir(CODECDUMP_PATH,S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IWGRP|S_IXGRP|S_IROTH|S_IXOTH)==0)
        {
            if(sys_chown(CODECDUMP_PATH,1000,1000)==-1)
            {
                ad_dev_loge(codec->dev,"%s chown error\n",__func__);
            }
        }
        else
        {
            ad_dev_loge(codec->dev,"%s mkdir\n",__func__);
        }
    }
...
static int msm8x16_wcd_spmi_probe(struct spmi_device *spmi)
{
	...
	if (pdata == NULL) {
		ad_dev_loge(&spmi->dev, "%s:Platform data failed to populate\n",
			__func__);
		goto rtn;
	}
